  ...
  <section id="recipe" class="container relative  py-16 px-64 flex flex-col">
      <div class="flex h-64 w-full rounded-lg bg-white shadow-md">
        ...
        <div class="p-6 flex flex-col justify-start">
          <contenteditable tag="p" class="text-gray-900 text-4xl font-medium mb-4" :class="isEditable ? 'hover:bg-blue-100' : 'cursor-auto'	" :contenteditable="isEditable" v-model="recipe.name" :noNL="true"/>
          <contenteditable tag="p" class="text-gray-700 text-xl mb-4" :class="isEditable ? 'hover:bg-blue-100' : 'cursor-auto'	" :contenteditable="isEditable" v-model="recipe.description" :noNL="true"/>
    ...
    <contenteditable tag="div" class="w-full max-w-none mt-8 p-4" :class="isEditable ? 'hover:bg-blue-100' : 'cursor-auto'" :contenteditable="isEditable" v-model="recipe.ingredients" :noNL="true"/>
    <contenteditable tag="div" class="w-full max-w-none mt-8 p-4 rounded-xl bg-slate-50 prose prose-slate prose-headings:block prose-headings:text-indigo-500" :class="isEditable ? 'hover:bg-blue-100' : 'cursor-auto'	" :contenteditable="isEditable" v-model="recipe.steps"/>
    <div class="fixed bottom-0 right-0 m-8" :class="isEditable ? 'hover:bg-blue-100' : 'cursor-auto'	" v-if="isCreator">
    ...
    <div class="fixed bottom-0 right-0 m-8" v-if="isNew">
      <button type="button" v-on:click="createRecipe" data-mdb-ripple="true" data-mdb-ripple-color="light" class="inline-flex items-center justify-center rounded-full px-2 mr-1 bg-green-600 text-white leading-normal uppercase shadow-md hover:bg-green-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-green-800 active:shadow-lg transition duration-150 ease-in-out w-12 h-12">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-5" viewBox="0 0 418 512" fill="currentColor">
