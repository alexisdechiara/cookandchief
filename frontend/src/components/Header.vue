  ...
  <header id="header">
    <nav class="navbar navbar-expand-lg navbar-light relative flex w-full flex-wrap items-center justify-between bg-gray-100 py-2 text-gray-500 shadow-lg hover:text-gray-700 focus:text-gray-700">
      <div class="container-fluid flex w-full flex-wrap items-center justify-between px-6">
        ...
        <div class="collapse navbar-collapse flex-grow items-center" id="navbarSupportedContent" v-if="isConnected">
        ...
        <div class="relative flex items-center">
          <ul class="navbar-nav list-style-none mr-auto flex flex-col pl-0" v-if="!isConnected">
            <li class="nav-item p-2 pr-0">
              <router-link class="nav-link p-0 text-gray-500 hover:text-gray-700 focus:text-gray-700" to="/login">Connexion</router-link>
            </li>
            <li class="nav-item p-2 pl-0">
              <router-link class="nav-link p-0 text-gray-500 hover:text-gray-700 focus:text-gray-700" to="/register">Inscription</router-link>
            </li>
          </ul>
          <span class="mr-2" v-else>{{ getUser().firstname }} {{getUser().lastname}}</span>
          <!-- Icon -->
          <div class="dropdown relative" v-if="isConnected">
            <a class="dropdown-toggle hidden-arrow flex items-center" href="#" id="dropdownMenuButton2" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            ...
            <ul class="dropdown-menu absolute left-auto right-0 z-50 float-left m-0 mt-1 hidden hidden min-w-max list-none rounded-lg border-none bg-white bg-clip-padding py-2 text-left text-base shadow-lg" aria-labelledby="dropdownMenuButton2">
              <li><a class="dropdown-item block w-full whitespace-nowrap bg-transparent py-2 px-4 text-sm font-normal text-gray-700 hover:bg-gray-100" href="#">Action</a></li>
              <li><a class="dropdown-item block w-full whitespace-nowrap bg-transparent py-2 px-4 text-sm font-normal text-gray-700 hover:bg-gray-100" href="#">Another action</a></li>
              <li><a class="dropdown-item block w-full whitespace-nowrap bg-transparent py-2 px-4 text-sm font-normal text-gray-700 hover:bg-gray-100" href="#">Something else here</a></li>
            </ul>
          </div>
        </div>
        <!-- Right elements -->
